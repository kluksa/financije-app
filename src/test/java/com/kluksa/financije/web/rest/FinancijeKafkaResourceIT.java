...
import com.kluksa.financije.config.KafkaProperties;
...
class FinancijeKafkaResourceIT {
    ...
    void setup() {
        ...
        FinancijeKafkaResource kafkaResource = new FinancijeKafkaResource(kafkaProperties);
    ...
    void producesMessages() throws Exception {
        restMockMvc
            .perform(post("/api/financije-kafka/publish/topic-produce?message=value-produce"))
    ...
    void consumesMessages() throws Exception {
        ...
        MvcResult mvcResult = restMockMvc
            .perform(get("/api/financije-kafka/consume?topic=topic-consume"))
            .andExpect(status().isOk())
